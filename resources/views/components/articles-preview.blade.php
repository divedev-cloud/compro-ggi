...
@if ($latestArticles->isNotEmpty())
    <section id="artikel" class="section-pad relative overflow-hidden">
        <div class="absolute top-0 left-1/4 w-96 h-96 bg-cyan-500/10 rounded-full blur-[130px] -z-10" aria-hidden="true">
        </div>
        <div class="absolute bottom-0 right-0 w-80 h-80 bg-violet-500/10 rounded-full blur-[120px] -z-10"
        ...
        <div class="max-w-7xl mx-auto px-5 sm:px-8 relative">
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                @foreach ($latestArticles as $i => $article)
                    <article
                        class="glass-card group overflow-hidden flex flex-col reveal-scale transition-all duration-300 hover:shadow-glow-cyan hover:-translate-y-1"
                        data-delay="{{ $i * 80 }}" data-tilt>

                        {{-- Thumbnail --}}
                        <a href="{{ route('artikel.show', $article->slug) }}"
                            class="block aspect-[16/9] overflow-hidden bg-gradient-to-br from-cyan-900/5 to-violet-900/5 flex-shrink-0 relative">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center">
                                    <svg class="w-12 h-12 text-cyan-600/20" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                            ...
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-ink-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                            </div>
                        </a>
                        ...
                        <div class="p-6 sm:p-7 flex flex-col flex-1">
                            <div class="flex items-center gap-3 mb-4 flex-wrap">
                                @if($article->category)
                                    <span
                                        class="text-[0.65rem] sm:text-xs font-bold text-violet-700 bg-violet-500/10 px-3 py-1.5 rounded-full uppercase tracking-wider">{{ $article->category->name }}</span>
                                @else
                                    <span
                                        class="text-[0.65rem] sm:text-xs font-bold text-cyan-700 bg-cyan-500/10 px-3 py-1.5 rounded-full uppercase tracking-wider">Artikel</span>
                                @endif
                            ...
                            <h3 class="font-display font-600 text-lg sm:text-xl text-ink-900 mb-3 line-clamp-2">
                                <a href="{{ route('artikel.show', $article->slug) }}"
                                    class="group-hover:text-cyan-600 transition-colors">
                                    {{ $article->title }}
                                </a>
                            </h3>
