...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-60 flex-shrink-0 bg-slate-900 flex flex-col overflow-hidden">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.articles.index') }}"
               class="{{ $isArticles ? 'bg-cyan-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors">
            ...
            <a href="{{ url('/') }}" target="_blank"
               class="text-slate-300 hover:bg-slate-800 hover:text-white flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-4 py-4 border-t border-slate-700">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-8 h-8 rounded-full bg-cyan-900 border border-cyan-700 flex items-center justify-center text-cyan-400 text-xs font-bold flex-shrink-0">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between flex-shrink-0">
            <div>
                <h1 class="text-slate-900 font-semibold text-lg leading-tight">{{ $title }}</h1>
                @if($subtitle)
